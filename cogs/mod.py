...
class Mod(commands.Cog):
    ...
    async def mute(self,ctx,member:discord.Member,reason=None):
        ...
        await ctx.send(embed=(await embed.generate(f"{member.name} has been muted",f"{ctx.mention} muted this user for: `{reason}`")))
    ...
    async def unmute(self,ctx,member:discord.Member,reason=None):
        ...
        await ctx.send(embed=(await embed.generate(f"{member.name} has been unmutted",f"{ctx.mention} unmutted this user for: `{reason}`")))
